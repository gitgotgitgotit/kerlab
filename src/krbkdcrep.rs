use asn1::{Tag, Integer, SequenceOf, ASN1, Application, OctetString, GeneralString, BitString};
use padata::PaData;
use base::{PrincipalName, Realm, LastReq, KerberosTime, TicketFlags, HostAddresses, PrincipalNameType};
use ticket::Ticket;
use yasna::{DERWriter, BERReader};
use error::{KerlabResult};
use encryption::{EncryptedData, KeyUsage, EncryptionKey, EType};
use std::str::FromStr;
...
impl EncKDCRepPart {
    pub fn new(keytype: EType, keyvalue: OctetString, srealm: &str, sname: &str, ticket_flags: u32) -> KerlabResult<Self> {
        Ok(
            EncKDCRepPart {
                key : Tag::new(EncryptionKey::new(
                    keytype, keyvalue
                )),
                last_req: Tag::new(LastReq::new()),
                nonce: Tag::new(0),
                key_expiration: Some(Tag::new(KerberosTime::default())),
                flags: Tag::new(BitString::from_bytes(ticket_flags.to_be_bytes().as_ref())),
                authtime: Tag::new(KerberosTime::default()),
                starttime: Some(Tag::new(KerberosTime::default())),
                endtime: Tag::new(KerberosTime::default()),
                renew_till: Some(Tag::new(KerberosTime::default())),
                srealm: Tag::new(GeneralString::from_str(srealm)?),
                sname: Tag::new(
                    PrincipalName::new(
                        PrincipalNameType::NtSrvInst,
                        vec![GeneralString::from_str(sname)?, GeneralString::from_str(srealm)?]
                    )
                ),
                caddr: None,
            }
