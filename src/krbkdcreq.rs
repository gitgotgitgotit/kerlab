...
impl KdcOptionsType {
    pub fn join(options: &[KdcOptionsType]) -> u32 {
        let mut result = 0;
        for e in options {
            result |= *e as u32;
        }
...
impl AsReq {
    /// constructor
    pub fn new(domain: &str, username: &str, options: &[KdcOptionsType], etype: Option<Vec<EType>>) -> KerlabResult<AsReq> {
        Ok(AsReq {
            inner: KdcReq {
                pvno: Tag::new(5),
...
impl TgsReq {
    pub fn new(domain: &str, username: &str, sname: PrincipalName, ap_req: &ApReq, options: &[KdcOptionsType], etype: Option<Vec<EType>>) -> KerlabResult<TgsReq> {
        Ok(TgsReq {
            inner: KdcReq {
                pvno: Tag::new(5),
...
impl KdcReqBody {
    pub fn new(cname: PrincipalName, domain: &str, sname: PrincipalName, kdc_options: u32, till: DateTime<Utc>, etype: Option<Vec<EType>>) -> KerlabResult<Self> {
        Ok(Self {
            ...
            etype: Tag::new(
                etype
                    .unwrap_or(vec![EType::Rc4Hmac, EType::Aes256CtsHmacSha196, EType::Aes128CtsHmacSha196])
                    .into_iter().map(|e| e as Integer).collect()
            ),
            addresses: None,
